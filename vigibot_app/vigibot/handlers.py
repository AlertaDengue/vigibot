...
from vigibot.data import get_alerta, get_geocode
...
disease_keyboard = [
    [KeyboardButton("/alerta " + d + " Rio de Janeiro")]
    for d in ["dengue", "chikungunya", "zika"]
]
disease_keyboard_markup = ReplyKeyboardMarkup(
    disease_keyboard, one_time_keyboard=True
)
...
def error(update, context):
    ...
    try:
        upd = (
            str(update)
            if update is None
            else json.dumps(update.to_dict(), indent=2, ensure_ascii=False)
        )
        message = (
            "An exception was raised while handling an update\n"
            f"<pre>update = {html.escape(upd)}"
            "</pre>\n\n"
            "<pre>context.chat_data ="
            f" {html.escape(str(context.chat_data))}</pre>\n\n"
            "<pre>context.user_data ="
        ...
        context.bot.send_message(
            ...
            text=f"Problem sending traceback {e}",
...
def bom_dia(update, context):
    ...
    update.message.reply_text(
        "Bom dia! " + user.first_name + emoj, parse_mode="Markdown"
    )
    update.message.reply_text(
        'Siga-me no <a href="https://twitter.com/evigilancia2">Twitter!</a>',
        parse_mode=ParseMode.HTML,
    )
    ...
    if not check_user_exists(user.id):
        ...
        update.message.reply_text(
            "Eu sou o Vigibot, minha missão é manter você informad@ sobre as"
            " arboviroses!"
        )
        update.message.reply_text(
            "Voce se importa de compartilhar sua localizaçao comigo? \nAssim"
            " posso te enviar informaçoes sobre o seu local!",
...
def on_inline_result_chosen(update, context):
    ...
    try:
        save_question(query, "Telegram", username, int(update.update_id))
    except Exception as e:
        context.bot.send_message(
            chat_id=DEVELOPER_CHAT_ID,
            text=(
                f"Problem saving inline query {query} from user"
                f" {username}: {e}"
            ),
            parse_mode=ParseMode.HTML,
        )
...
def alerta(update, context):
    ...
    if context.args == []:
        update.message.reply_text(
            "Por favor especifique uma doença e uma cidade.\nPor exemplo:"
            " /alerta dengue niteroi"
        )
        return
    doenca = context.args[0].lower()
    if doenca not in ["dengue", "chik", "chikungunya", "zika"]:
        update.message.reply_text(
            "Escolha uma destas doenças:", reply_markup=disease_keyboard_markup
        )
        return
    ...
    # print(doenca, cidade)
    module_logger.info(
        "%s fez uma consulta de alerta sobre %s em %s",
        usr_chat_id,
        doenca,
        cidade,
    )
    ...
    if not isinstance(gc, int):
        update.message.reply_text(
            emojize(uni_emoji["thinking"])
            + ' Nao conheço a cidade "'
            + cidade
            + '". Voce digitou os acentos?'
        )
    ...
    if alrt is None:
        update.message.reply_text(
            emojize(uni_emoji["thinking"])
            + " Nao temos esta informaçao no momento."
        )
        return
    # print(alrt)
    niveis = {
    ...
    update.message.reply_text(
        f'''
        Para maiores detalhes, consulte o <a href="
            https://info.dengue.mat.br/alerta/{gc}/dengue">Infodengue
        </a>.''',
        parse_mode=ParseMode.HTML,
    )
    word = random.choice(
    ...
    try:
        tweetapi.update_status(
            f"Oi {word}, estamos no nivel {niveis[alrt[0]]}, para a"
            f" {doenca} em {cidade}.\nPara maiores detalhes, consulte o"
            f" https://info.dengue.mat.br/alerta/{gc}/dengue"
        )
    except Exception:
...
def unknown(update, context):
    emoj = emojize(":smiley:", use_aliases=True)
    update.message.reply_text(
        "Desculpa, Nao conheço este comando. " + emoj, parse_mode="Markdown"
    )
...
def location(update, context):
    ...
    update.message.reply_text(
        emojize(":thumbs_up:", use_aliases=True)
        + "\nObrigado "
        + user.first_name
...
def check_user_exists(tid):
    ...
    with conexao.cursor() as cursor:
        cursor.execute(
            f"select * from bot_users where telegram_uid={tid}"
        )  # , {'tid': tid}))
        res = cursor.fetchone()
...
def add_user(user):
    ...
    with conexao.cursor() as cursor:
        cursor.execute(
            "insert into bot_users(telegram_uid, first_name, last_name)"
            f" values({user.id}, '{user.first_name}','{user.last_name}');"
        )
...
def add_location(user, location):
    ...
    with conexao.cursor() as cursor:
        sql = (
            f"update bot_users set latitude= {location.latitude}, longitude="
            f" {location.longitude} where telegram_uid= {user.id};"
        )
        cursor.execute(sql)
