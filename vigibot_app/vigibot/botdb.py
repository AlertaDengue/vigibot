...
def create_pergunta_table():
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            " table_name='pergunta' limit 5;"
        )
        if not cur.rowcount:
            cur.execute(
                "create table pergunta(id bigserial, username varchar(32),"
                " network varchar(16), pergunta text, datetime timestamp,"
                " msgid bigint;"
            )
...
def save_question(pergunta, rede, userid, msgid):
    ...
    with conn.cursor() as cursor:
        sql = (
            "insert into pergunta(network,username,pergunta,msgid)"
            f" values('{rede}','{userid}','{pergunta}',{msgid});"
        )
        cursor.execute(sql)
    conn.commit()
