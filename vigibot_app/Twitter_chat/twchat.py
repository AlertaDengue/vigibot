...
from vigibot.botdb import is_new_id, save_question
        ...
        logic_adapters=[
            {
                ...
                "default_response": (
                    "Não entendi. Me pergunte algo sobre dengue, zika ou"
                    " chikungunya, ou visite:"
                ),
...
def reply_mentions(api, keywords, since_id, Cbot):
    ...
    for tweet in tweepy.Cursor(
        ...
        if not is_new_id(
            int(tweet.id)
        ):  # check if this tweet has already been answered.
            continue
        ...
        try:
            ...
            logger.error(
                f"Não consegui salvar mensagem {msg} do Twitter:\n {e}"
            )
...
def main():
    ...
    since_id = 1
    while True:
        since_id = reply_mentions(
            api, keywords, since_id=since_id, Cbot=chatbot
        )
        logger.info("Waiting...")
        time.sleep(60)
